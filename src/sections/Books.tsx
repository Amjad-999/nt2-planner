...
export default function Books(_: {}) {
  ...
  return (
    <div style={{ padding: '24px 28px 60px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {BOOKS.map((b) => {
        const doneUnits = bookUnits[b.id] ?? []
        const pct = Math.round((doneUnits.length / b.units.length) * 100)
        return (
          <div key={b.id} style={{ background:'var(--glass-bg)', backdropFilter:'blur(16px)', WebkitBackdropFilter:'blur(16px)', border:'1px solid var(--glass-border)', borderInlineStart:`4px solid ${b.ic}`, borderRadius:'var(--r)', padding:18, boxShadow:'var(--elev-1), inset 0 1px 0 var(--glass-hi)', marginBottom:14 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:10, marginBottom:8 }}>
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:8, fontWeight:600, color:'var(--text)', fontSize:'1.02rem' }}>
                  <span style={{ width:32, height:32, background:b.bg, color:b.ic, borderRadius:8, display:'inline-flex', alignItems:'center', justifyContent:'center' }}>{b.icon}</span>
                  {b.title}
                </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:5 }}>
              {b.units.map((u, i) => (
                <label key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 9px', background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:6, cursor:'pointer', fontSize:'.86rem', color:'var(--text)' }}>
                  <input type="checkbox" checked={doneUnits.includes(i)} onChange={() => toggleBookUnit(b.id, i)} style={{ accentColor: b.ic }} aria-label={u} />
                  <span>{u}</span>
                </label>
              ))}
            </div>
