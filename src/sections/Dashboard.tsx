...
export default function Dashboard({ onOpenStudyTime }: Props) {
  ...
  insights.push({ kind:'', icon:'🧭', title:`مهارتك الأضعف: ${SKILL_AR[wkSk]}`, desc:`أعلى نتيجة لديك في ${SKILL_AR[wkSk]} هي ${wkBest}٪. خصّص جلسة ٣٠ دقيقة لها اليوم.` })
  ...
  if (remDays>0) insights.push({ kind:projected>=PASS_THRESHOLD?'good':'warn', icon:'🔮', title:'توقّع جاهزيتك يوم الامتحان', desc:`بمعدّلك الحالي (${bestNT2}٪) وبواقع ${remDays} يوم، أنت في طريقك إلى ~${projected}٪. ${projected>=PASS_THRESHOLD?'مرشّح للنجاح إن حافظت على الإيقاع.':'تحتاج رفع الكثافة لتجاوز عتبة ٦٥٪.'}` })
